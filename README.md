...
**An AI agent that automates first-line e-commerce customer support and proves the ROI. It's grounded in your own catalog and policies, with a provider-agnostic LLM layer.**
...
- **Answers inquiries with citations** using RAG over the store's product catalog and policy documents,
...
- **Python 3.10+**, with **FastAPI** + **Jinja2** + **htmx** as a single-container web app (chat UI + dashboard), no JS build step.
...
Single container, deployable anywhere that runs a Docker image. The container needs **no secrets** to
